class PratosController < ApplicationController
  ...
  before_action :set_restaurantes, only: [:edit, :new]
  ...
  private
    ...
    def set_restaurantes
      @restaurantes = Restaurante.all.map do |restaurante|
        [restaurante.nome, restaurante.id.to_s]
      end || []
    end
    ...
    def prato_params
      params.require(:prato).permit(:restaurante_id, :nome, :preco)
    end
end
